...
class ResumeController extends Controller
{
    ...
    public function update(Request $request, Resume $resume){
        ...
        // Validate image
        $request->validate([
            'diploma' => 'image|mimes:png,jpeg,jpg,svg|max:2048'
            ]);

            $diploma = $request->file('diploma');
            if($diploma){
            Storage::delete($resume->profile->diploma); // delete image recently
            $diplomaUrl = $diploma->store('public/diplomas'); // update new image
        } else {
            $diplomaUrl = $resume->profile->diploma;
        }
    ...
    public function destroy(Resume $resume){
        ...
        Storage::delete($resume->profile->diploma);
    ...
    public function deleteDiploma(Resume $resume){
        Storage::delete($resume->profile->diploma);
        $resume->profile->update([
            'diploma' => null
        ]);
