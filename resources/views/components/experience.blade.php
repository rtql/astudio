<div class="accordion-item">
    <h2 class="accordion-header" id="headingTwo">
        ...
        <form action="/experience/{{ $id }}/delete" method="POST" onsubmit="return confirm('Подтвердите удаление {{ $employer }}');">
            @csrf
            @method('delete')
        ...
        <div class="accordion-body">
            <div class="d-lg-flex">
                ...
                <div class="form-floating mb-3 half-input pe-lg-3">
                    <input form="edit-form" type="text" value="{{ $employer ?? ""}}" name="employer[]" class="form-control" id="floating-school" placeholder="Организация" required>
                    <label for="floating-school">Организация</label>
                </div>
                <div class="form-floating mb-3 half-input">
                    <input form="edit-form" type="text" value="{{ $job ?? ""}}" name="job[]" class="form-control" id="work" placeholder="Должность" required>
            ...
            <div class="d-lg-flex">
                ...
                <label for="start-date-exp-{{ $id }}" class='mb-2'>Начало работы</label>
                    <div class="input-group mb-3">
                        ...
                        <input form="edit-form" type="text" name="start_date_exp[]" id="start-date-exp-{{ $id }}" class="form-control date-picker" value="{{ $start_date_exp }}" placeholder="Выберите дату">
                    </div>
                </div>

                <div class="mb-3 half-input">
                <label for="end-date-exp-{{ $id }}" class='mb-2'>Окончание</label>
                    <div class="input-group mb-3">
                        <label class="input-group-text" for="end-date-exp-{{ $id }}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" style="fill: rgba(0, 0, 0, 1);transform: ;msFilter:;"><path d="M7 11h2v2H7zm0 4h2v2H7zm4-4h2v2h-2zm0 4h2v2h-2zm4-4h2v2h-2zm0 4h2v2h-2z"></path><path d="M5 22h14c1.103 0 2-.897 2-2V6c0-1.103-.897-2-2-2h-2V2h-2v2H9V2H7v2H5c-1.103 0-2 .897-2 2v14c0 1.103.897 2 2 2zM19 8l.001 12H5V8h14z"></path></svg>
                        </label>
                        <input form="edit-form" type="text" name="end_date_exp[]" id="end-date-exp-{{ $id }}" class="form-control date-picker" value="{{ $end_date_exp }}" placeholder="Выберите дату">
                    </div>
                </div>
            ...
            <div class="form-floating mb-3">
                <textarea form="edit-form" name="experience_desc[]" class="form-control" id="job-description-{{ $id }}" placeholder="Описание" required style="height: 150px">{{ $description ?? ""}}</textarea>
                <label for="job-description-{{ $id }}">Описание</label>
            </div>
        </div>
    </div>
</div>